...
tnode_t* c_lvalue( void ) {
    tnode_t* retval;
    if( retval = checkreg() ) {
        printf( "checkreg() succeeded\n" );
        //scan( &g_token );
        return retval;
    }
    if( retval = checkint() ) {
        printf( "checkint() succeeded\n" );
        return retval;
    }
    printf( "got here\n" );
    exit( 1 );
    scan( &g_token );
    if( g_token.type != T_PERCENT ) {
        printf( "Syntax error: c_lvalue() unknown token type (%% expected): %d\n", g_token.type );
        exit( 1 );
    }
...
tnode_t* expr( int prec ) {
    ...
    //toktype = g_token.type;
    if( g_token.type == T_NL ) return left;
    right = expr( 0 );
    /*
    ...
    */
    int operation = 0;
    if( g_token.type == T_INSTR_LVR ) operation = OP_LVR;
    retval = mknode( operation, left, right, g_token.val );
    return retval;
}
